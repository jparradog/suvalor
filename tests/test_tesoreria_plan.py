...
@pytest.mark.parametrize("tag", ["mi cuenta", " corto "])
def test_plan_tesoreria_rechaza_tag_no_canonico_con_account(tmp_path, tag):
    with pytest.raises(ValueError, match="--account requiere --tag canonico"):
        construir_plan_tesoreria(
            base=tmp_path,
            desde_iso="2026-01-01",
